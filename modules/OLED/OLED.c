...
void IIC_Wait_Ack()     //等待响应
{
    OLED_SCLK_Set();    //P3OUT|=BIT5
    OLED_SCLK_Clr();    //P3OUT&=~BIT5
}
void Write_IIC_Byte(u8 IIC_Byte)     //通过IIC通信
{
    u8 i;
    u8 m,da;
...
void Write_IIC_Command(u8 IIC_Command)   //向OLED写入命令
{
   IIC_Start();
   Write_IIC_Byte(0x78);
   IIC_Wait_Ack();
...
void Write_IIC_Data(u8 IIC_Data)     //写数据
{
    IIC_Start();
    Write_IIC_Byte(0x78);
    IIC_Wait_Ack();
...
void OLED_WR_Byte(unsigned dat,unsigned cmd)    //OLED写入字节，就是引用上面两个函数
{
    if(cmd)
        Write_IIC_Data(dat);        //数据
    else
        Write_IIC_Command(dat);     //命令
}
void fill_picture(u8 fill_Data)      //图片
{
    u8 m,n;
    for(m=0;m<8;m++)
    {
        OLED_WR_Byte(0xb0+m,0);     //页地址
        OLED_WR_Byte(0x00,0);       //设置低八位
        OLED_WR_Byte(0x10,0);       //设置高八位
...
void OLED_Set_Pos(u8 x, u8 y)     //设置显示位置
{
...
num的数字可为变量，例：OLED_ShowNum(0,1,date,4,8);
*/
...
void OLED_ShowCHinese(u8 x,u8 y,u8 no)
{
    ...
    for(t=0;t<16;t++)
    {
        OLED_WR_Byte(DPJ[2*no][t],OLED_DATA);
        adder+=1;
    }
    OLED_Set_Pos(x,y+1);
    for(t=0;t<16;t++)
    {
        OLED_WR_Byte(DPJ[2*no+1][t],OLED_DATA);
        adder+=1;
    }
}
//绘制图像
